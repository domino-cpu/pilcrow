...
function SubmitButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="inline-flex items-center justify-center gap-2 bg-[var(--color-blush-deep)] px-7 py-3.5 font-body text-sm font-medium tracking-wide text-[var(--color-bg)] transition-[filter] duration-200 hover:brightness-95 disabled:cursor-not-allowed disabled:opacity-60"
    >
...
function FieldError({ id, message }: { id: string; message?: string }) {
  ...
  return (
    <p id={id} role="alert" className="mt-1.5 text-sm text-[var(--color-blush-deep)]">
      {message}
    </p>
  );
}
