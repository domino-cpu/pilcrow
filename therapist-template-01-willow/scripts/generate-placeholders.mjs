...
function ogSVG(w, h) {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}">
    ...
    <text x="80" y="${h * 0.42}" font-family="Georgia, serif" font-size="64" fill="${INK}">Sarah Chen, PhD, LMFT</text>
    <text x="80" y="${h * 0.56}" font-family="Georgia, serif" font-size="30" fill="${SAGE}" font-style="italic">Therapy for anxiety, trauma, and life transitions</text>
    <text x="80" y="${h * 0.82}" font-family="Arial, sans-serif" font-size="22" letter-spacing="3" fill="${INK}" opacity="0.6">PORTLAND, OREGON</text>
  </svg>`;
}
