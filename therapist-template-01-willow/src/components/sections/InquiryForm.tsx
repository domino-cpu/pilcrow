...
function SubmitButton() {
  ...
  return (
    <button
      type="submit"
      disabled={pending}
      className="inline-flex items-center justify-center gap-2 bg-[var(--color-terracotta)] px-7 py-3.5 font-body text-sm font-medium tracking-wide text-[var(--color-bg)] transition-colors duration-200 hover:bg-[var(--color-terracotta-deep)] disabled:cursor-not-allowed disabled:opacity-60"
    >
...
function FieldError({ id, message }: { id: string; message?: string }) {
  ...
  return (
    <p id={id} role="alert" className="mt-1.5 text-sm text-[var(--color-terracotta-deep)]">
      {message}
    </p>
  );
}
  ...
  return (
    <form action={formAction} className="space-y-6" noValidate>
      ...
      {state.status === "error" && state.message && (
        <p role="alert" className="text-sm text-[var(--color-terracotta-deep)]">
          {state.message}
        </p>
      )}
